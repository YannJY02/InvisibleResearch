...
privacy, and full-cell round-trip checks pass. A failed run leaves the previous
formal artifact untouched.

```{r}
#| label: pre-promotion-validation

has_source_field <- function(prefix, field) {
  any(startsWith(output_columns, paste0(prefix, "_")) &
...
run_metadata <- list(
  ...
  openalex = if (run_mode == "full") {
    ...
    list(cache_file = basename(openalex_cache_path), reused = openalex_cache_hit)
  },
  crossref = if (run_mode == "full") {
    ...
    list(cache_file = basename(crossref_cache_path), reused = crossref_cache_hit)
  },
  runtime = list(
    r = R.version.string,
