...
Inputs (defaults):
- SCImago CSV: $DATA_ROOT/raw/scimagojr_communication_journals.csv
- OpenAlex Parquet: $DATA_ROOT/processed/openalex_merged.parquet

Outputs under the owner's ignored artifacts directory:
- scim_openalex_coverage_summary.csv
  Columns: total_journals, covered_journals, unmatched_journals, coverage_rate
- scim_openalex_unmatched_journals.csv
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--scimago-csv", "--scimago_csv", default=None, help="Path to SCImago CSV (semicolon-separated)")
    parser.add_argument("--openalex-parquet", "--openalex_parquet", default=None, help="Path to OpenAlex merged Parquet")
    parser.add_argument("--output-dir", "--output_dir", default=None, help="Directory to write owner-local artifacts")
    parser.add_argument("--name-suffix", default="", help="Optional suffix retained by a notebook adapter")
...
def main() -> None:
    ...
    scimago_csv = Path(args.scimago_csv) if args.scimago_csv else data_root / "raw" / "scimagojr_communication_journals.csv"
    openalex_parquet = Path(args.openalex_parquet) if args.openalex_parquet else data_root / "processed" / "openalex_merged.parquet"
    output_dir = Path(args.output_dir) if args.output_dir else Path(__file__).parents[1] / "artifacts"
    ensure_output_dir(str(output_dir))

    scim_df = load_scimago_items(str(scimago_csv))
    openalex_issn = build_openalex_issn_set(str(openalex_parquet))
    openalex_titles = build_openalex_title_set(str(openalex_parquet))
    ...
    summary_path = output_dir / f"scim_openalex_coverage_summary{suffix}.csv"
    unmatched_path = output_dir / f"scim_openalex_unmatched_journals{suffix}.csv"
    ...
    print(json.dumps({
        "outputs": {
            "coverage_summary_csv": str(summary_path),
            "unmatched_journals_csv": str(unmatched_path),
        },
        "summary": out,
    }, ensure_ascii=False))
