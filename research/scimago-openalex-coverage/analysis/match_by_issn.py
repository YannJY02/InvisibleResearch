...
def main():
    ...
    parser.add_argument("--scimagojr-csv", "--scimagojr_csv", default=None)
    parser.add_argument("--openalex-parquet", "--openalex_parquet", default=None)
    parser.add_argument("--year-start", "--year_start", type=int, default=2020)
    parser.add_argument("--year-end", "--year_end", type=int, default=2025)
    parser.add_argument("--output-dir", "--output_dir", default=None)
    parser.add_argument("--per-journal", "--per_journal", action="store_true", help="emit per-journal counts as well")
    args = parser.parse_args()

    data_root = resolve_data_root()
    scimagojr_csv = Path(args.scimagojr_csv) if args.scimagojr_csv else data_root / "raw" / "scimagojr_communication_journals.csv"
    ...
    # Load SCImago CSV
    scim_df = pd.read_csv(scimagojr_csv, sep=";", dtype=str, low_memory=False)
    ...
    oa_head = pd.read_parquet(openalex_parquet, columns=None, engine="pyarrow").head(5)
    ...
    oa_df = pd.read_parquet(openalex_parquet, columns=[c for c in needed_cols if c in oa_head.columns], engine="pyarrow")
    ...
    # Outputs
    summary_path = output_dir / "issn_match_summary_by_year.csv"
    ...
    if args.per_journal:
        ...
        per_journal_path = output_dir / "issn_match_by_journal.csv"
    ...
    print(json.dumps({
        ...
        "outputs": {
            "summary_by_year_csv": str(summary_path),
            "per_journal_csv": str(per_journal_path) if per_journal_path else None,
        }
    }, ensure_ascii=False))
